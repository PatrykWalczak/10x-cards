...
export class GenerationService {
   ...
   */
  async updateGenerationStats(generationId: number, stats: GenerationStatsUpdateParams): Promise<void> {
    const { error } = await this.supabase.from("generations").update(stats).eq("id", generationId);
  ...
  async logGenerationError(params: GenerationErrorLogParams): Promise<void> {
    const { error } = await this.supabase.from("generation_error_logs").insert(params);
   ...
   */
  async getGenerationById(generationId: number, userId: string): Promise<GenerationRecord | null> {
