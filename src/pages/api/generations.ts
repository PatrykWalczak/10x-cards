...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    try {
      ...
      console.log(
        `Koszt generacji fiszek: $${costUsage.totalCost.toFixed(4)}, użyte tokeny: ${costUsage.tokenUsage.total}`
      );
      ...
      console.error("Error details:", {
        ...
        code:
          aiError && typeof aiError === "object" && "code" in aiError ? (aiError as { code: unknown }).code : undefined,
        stack: aiError instanceof Error ? aiError.stack : undefined,
      });

      let errorCode =
        (aiError && typeof aiError === "object" && "code" in aiError
          ? (aiError as { code: string }).code
          : undefined) || "AI_ERROR";
      let errorMessage =
        (aiError instanceof Error ? aiError.message : String(aiError)) || "Nie udało się wygenerować fiszek";
      let statusCode = 503;
      ...
      } else if (aiError instanceof Error && aiError.message && aiError.message.includes("Rate limit exceeded")) {
        // Obsługa rate limit dla ogólnych błędów
        statusCode = 429;
        errorCode = "RATE_LIMIT_ERROR";
        errorMessage = "Przekroczono limit zapytań do usługi AI. Spróbuj ponownie za kilka minut.";
      } else if (aiError instanceof Error && aiError.message && aiError.message.includes("No endpoints found")) {
