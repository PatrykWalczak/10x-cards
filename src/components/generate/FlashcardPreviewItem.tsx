...
const FlashcardPreviewItem: React.FC<FlashcardPreviewItemProps> = ({
  flashcard,
  onAction
}) => {
  ...
  const getStatusBadge = () => {
    switch (flashcard.status) {
      case 'accepted':
        return <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-green-100 text-green-800 rounded-full">Zaakceptowana</span>;
      case 'edited':
        return <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">Edytowana</span>;
      case 'rejected':
        return <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-red-100 text-red-800 rounded-full">Odrzucona</span>;
      default:
        return <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full">Oczekująca</span>;
    }
  };
    ...
    <Card className={`transition-all duration-200 ${getStatusColor()}`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm bg-white p-3 rounded border min-h-[60px] break-words">
            {displayFront}
          </p>
        </div>
