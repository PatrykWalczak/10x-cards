...
const EditFlashcardModal: React.FC<EditFlashcardModalProps> = ({ isOpen, flashcard, onSave, onClose }) => {
  ...
  useEffect(() => {
    if (flashcard) {
      const initialData = {
        front: flashcard.isEdited ? flashcard.editedFront || flashcard.front : flashcard.front,
        back: flashcard.isEdited ? flashcard.editedBack || flashcard.back : flashcard.back,
      };
  ...
  useEffect(() => {
    if (flashcard) {
      const originalFront = flashcard.isEdited ? flashcard.editedFront || flashcard.front : flashcard.front;
      const originalBack = flashcard.isEdited ? flashcard.editedBack || flashcard.back : flashcard.back;

      setHasChanges(formData.front !== originalFront || formData.back !== originalBack);
    }
  }, [formData, flashcard]);
  ...
  const handleClose = () => {
    if (hasChanges) {
      const confirmClose = window.confirm("Masz niezapisane zmiany. Czy na pewno chcesz zamknąć okno?");
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px]" onKeyDown={handleKeyDown}>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              {errors.front && <span className="text-sm text-red-500">{errors.front}</span>}
          ...
          <div className="space-y-2">
            ...
            />
            <div className="flex justify-between items-center">
              {errors.back && <span className="text-sm text-red-500">{errors.back}</span>}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!hasChanges && Object.keys(errors).length === 0}>
